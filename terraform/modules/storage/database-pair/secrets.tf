...
resource "aws_secretsmanager_secret" "db-pv-secret" {
  name = "${var.environment}/rds/pv/"
  # Once the secret is deleted, we cant get it back.
  # If we don't do this, then a new secret can be made with the same name until the recovery window is over
  recovery_window_in_days = 0
...
resource "aws_secretsmanager_secret_version" "pv-version" {
  secret_id = aws_secretsmanager_secret.db-pv-secret.id
  secret_string = jsonencode(
    {
      username : "main",
